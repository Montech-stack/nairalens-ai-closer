  ...
  return (
    <div className="space-y-6 max-w-7xl">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KPI label="Active Campaigns" value={String(list.filter((c) => c.status === "active").length)} icon={TrendingUp} />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <div className="glass rounded-2xl p-5">
          ...
          {meta.length === 0 ? (
            <p className="text-sm text-muted-foreground py-6 text-center">No Meta campaigns yet. Add one below.</p>
          ) : (
              ...
              {meta.map((c) => (
                <div key={c.id}>
                  <div className="flex justify-between text-sm mb-1">
                    <span>{c.name}</span>
                    <span className="font-mono text-gold">{c.leads_count} leads · {formatNaira(c.cpl)}</span>
                  </div>
                  <div className="h-1.5 bg-noir-elevated rounded-full"><div className="h-full bg-gold rounded-full" style={{ width: `${Math.min(100, (Number(c.leads_count) / 100) * 100)}%` }} /></div>
                </div>
              ))}
            </div>
          )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs font-mono text-muted-foreground uppercase">
                <th className="p-2">Name</th><th>Platform</th><th>Date Range</th><th>Budget</th><th>Leads</th><th>CPL</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              {list.length === 0 ? (
                ...
                <tr key={c.id} className="border-t border-border">
                  ...
                  <td className="text-xs font-mono">{c.date_start} → {c.date_end}</td>
...
function Metric({ label, value }: any) {
  return <div className="bg-noir-elevated rounded-lg p-3"><div className="text-[10px] font-mono text-muted-foreground uppercase">{label}</div><div className="font-mono text-xl text-gold mt-1">{value}</div></div>;
}
  ...
  useEffect(() => {
    ...
    (async () => {
      const [a, b, c, d] = await Promise.all([
        supabase.from("conversations").select("*", { count: "exact", head: true }).eq("role", "lead"),
...
function NewCampaignSheet({ onSaved }: any) {
  ...
  const [f, setF] = useState({ name: "", platform: "Meta", budget: "", date_start: "", date_end: "", leads_count: 0, cpl: 0, status: "active" });
  const save = async () => {
    if (!user || !f.name) return;
    await supabase.from("campaigns").insert({ user_id: user.id, ...f, budget: Number(f.budget) || 0 });
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild><Button className="bg-gold text-noir hover:bg-gold/90"><Plus className="w-4 h-4 mr-1" /> New Campaign</Button></SheetTrigger>
      <SheetContent className="bg-noir-elevated border-gold/30 overflow-y-auto">
        <SheetHeader><SheetTitle className="font-serif">New Campaign</SheetTitle></SheetHeader>
        <div className="space-y-3 mt-4">
          <FE label="Name"><Input value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} /></FE>
          <FE label="Platform">
            <Select value={f.platform} onValueChange={(v) => setF({ ...f, platform: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>{["Meta", "Google", "WhatsApp", "Other"].map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
            </Select>
          </FE>
          <FE label="Budget (₦)"><Input type="number" value={f.budget} onChange={(e) => setF({ ...f, budget: e.target.value })} /></FE>
          <div className="grid grid-cols-2 gap-2">
            <FE label="Start"><Input type="date" value={f.date_start} onChange={(e) => setF({ ...f, date_start: e.target.value })} /></FE>
            <FE label="End"><Input type="date" value={f.date_end} onChange={(e) => setF({ ...f, date_end: e.target.value })} /></FE>
          </div>
          <Button onClick={save} className="w-full bg-gold text-noir hover:bg-gold/90">Save Campaign</Button>
        </div>
      </SheetContent>
    </Sheet>
  );
