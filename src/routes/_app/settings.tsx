...
const PERSONAS = [
  {
    ...
    desc: "Out-closes any human. Straight-Line persuasion, urgency engineering, zero hesitation, 24/7.",
  },
  {
    id: "diaspora_whisperer",
    title: "The Diaspora Whisperer",
    desc: "Patient, evidence-driven, multi-timezone. Converts UK/US/Canada-based buyers a top human can't reach in time.",
  },
  {
    id: "hni_concierge",
    title: "The HNI Concierge",
    desc: "Luxury positioning for ultra-HNI deals. Exclusivity, scarcity, and white-glove tone built for ₦500M+ tickets.",
  },
];
...
function Settings() {
  ...
  const load = async () => {
    ...
    if (w) {
      ...
      setWaForm({
        ...
        business_phone: w.business_phone || "",
      });
    }
  };
  useEffect(() => {
  ...
  const save = async () => {
    ...
    await supabase
      ...
      .update({
        ...
        site_visit_cta: profile.site_visit_cta,
        price_objection_response: profile.price_objection_response,
  ...
  const saveWhatsApp = async () => {
    ...
    setWaSaving(true);
    const verifyToken =
      wa.verify_token || Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2);
  ...
  const addTag = async () => {
    ...
    const { data } = await supabase
      .from("market_tags")
      .insert({ user_id: user.id, tag_text: newTag.trim() })
      .select()
      .single();
      ...
      <div className="glass-gold rounded-2xl p-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground max-w-xl mt-0.5">
                Link your WhatsApp Business number. Every inbound message gets qualified and closed
                by the AI automatically.
              </p>
            </div>
          </div>
          <span
            className={`px-3 py-1.5 rounded-full text-xs font-mono border shrink-0 ${isConnected ? "bg-success/15 text-success border-success/40" : "bg-border/30 text-muted-foreground border-border"}`}
          >
        ...
        <div className="rounded-xl border border-gold/25 bg-noir-elevated/40 p-5 flex flex-col gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Connect via Twilio Sandbox (free, instant) or Meta Cloud API (production). NairaLens
              handles every inbound chat automatically.
            </p>
          </div>
          {wa.business_phone && (
            ...
            <div className="space-y-2 text-xs">
              <div className="flex items-center justify-between gap-2 bg-noir rounded-lg px-3 py-2 border border-border">
                <span className="font-mono text-muted-foreground truncate">
                  <span className="text-gold">{isTwilio ? "Twilio" : "Meta"} URL: </span>
                  {webhookUrl}
                </span>
                <button
                  onClick={() => copyToClipboard(webhookUrl, "Webhook URL")}
                  className="shrink-0 text-gold hover:opacity-80"
                >
              ...
              {!isTwilio && (
                <div className="flex items-center justify-between gap-2 bg-noir rounded-lg px-3 py-2 border border-border">
                  <span className="font-mono text-muted-foreground">
                    Verify token: <span className="text-foreground">{wa.verify_token}</span>
                  </span>
                  <button
                    onClick={() => copyToClipboard(wa.verify_token, "Verify token")}
                    className="shrink-0 text-gold hover:opacity-80"
                  >
                    <Copy className="w-3.5 h-3.5" />
                  </button>
          ...
          <div className="flex gap-2 mt-auto">
            <Button
              ...
              className="flex-1 bg-gold text-noir hover:bg-gold/90 font-semibold"
            >
        ...
        {wa.last_event_at && (
          <div className="mt-5 pt-4 border-t border-border flex items-center gap-2 text-xs text-success font-mono">
            <CheckCircle2 className="w-3.5 h-3.5" /> Last lead received{" "}
            {new Date(wa.last_event_at).toLocaleString()}
          </div>
        )}
      </div>
          ...
          <div className="glass rounded-2xl p-6 space-y-4">
            ...
            <Field label="AI Model">
              <Select
                ...
                onValueChange={(v) => setProfile({ ...profile, ai_model: v })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="groq/llama-3.3-70b-versatile">
                    Groq · Llama 3.3 70B Versatile (Recommended — best reasoning + speed)
                  </SelectItem>
                  ...
                  <SelectItem value="groq/deepseek-r1-distill-llama-70b">
                    Groq · DeepSeek R1 Distill 70B (Deep reasoning)
                  </SelectItem>
            ...
            <p className="text-xs text-muted-foreground -mt-2">
              Whichever you pick is the primary brain. If it fails, NairaLens automatically falls
              back to the other provider so replies never stop.
            </p>
            ...
            <Field label="Max Follow-up Attempts">
              <Input
                ...
                min={1}
                max={10}
                value={profile.max_followups}
                onChange={(e) => setProfile({ ...profile, max_followups: Number(e.target.value) })}
              />
            </Field>
            <Field label={`Response Delay — ${profile.response_delay}s`}>
              <Slider
                value={[profile.response_delay]}
                max={30}
                step={1}
                onValueChange={(v) => setProfile({ ...profile, response_delay: v[0] })}
              />
            </Field>
          </div>
        </div>
        ...
        <div className="space-y-5">
          <div className="glass rounded-2xl p-6">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Live facts injected into every active AI session.
            </p>
            <div className="flex flex-wrap gap-2 mb-4 min-h-[40px]">
              ...
              {tags.map((t) => (
                ...
                >
                  {t.tag_text}
                  <button onClick={() => delTag(t.id)} className="hover:text-foreground">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
            ...
            <div className="flex gap-2">
              <Input
                ...
                placeholder="Add a market fact…"
                onKeyDown={(e) => e.key === "Enter" && addTag()}
              />
              <Button onClick={addTag} className="bg-gold text-noir hover:bg-gold/90">
                Add
              </Button>
            </div>
          </div>
          ...
          <div className="glass rounded-2xl p-6 space-y-4">
            ...
            <Field label="Trigger phrases (comma-separated)">
              <Textarea
                rows={2}
                value={profile.closing_triggers}
                onChange={(e) => setProfile({ ...profile, closing_triggers: e.target.value })}
                placeholder="ready to buy, when can I see, send account details"
              />
            </Field>
            <Field label="Site Visit CTA template">
              <Textarea
                rows={3}
                value={profile.site_visit_cta}
                onChange={(e) => setProfile({ ...profile, site_visit_cta: e.target.value })}
                placeholder="Sir, I'll personally walk you through Plot 14 tomorrow at 11am — shall I confirm?"
              />
            </Field>
            <Field label="Price Objection Response">
              <Textarea
                rows={3}
                value={profile.price_objection_response}
                onChange={(e) =>
                  setProfile({ ...profile, price_objection_response: e.target.value })
                }
                placeholder="Our pricing reflects the C of O title and Maitama 2027 infrastructure plan…"
              />
            </Field>
            <Button
              onClick={save}
                  ...
                  <div className="flex items-center justify-between gap-2 bg-noir rounded-lg px-3 py-2 border border-border text-xs">
                    <span className="font-mono text-muted-foreground truncate">
                      ...
                      <span className="text-foreground">
                        {typeof window !== "undefined"
                          ? `${window.location.origin}/api/public/twilio-webhook`
