      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[260px]">
          ...
          <Input
            ...
            className="pl-10 focus:ring-gold"
          />
        </div>
        ...
        <Select value={source} onValueChange={setSource}>
          ...
          <SelectContent>
            {["all", "Meta Ads", "WhatsApp", "Referral", "Direct"].map((s) => (
              <SelectItem key={s} value={s}>
                {s}
              </SelectItem>
            ))}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((l) => (
                <tr
                  key={l.id}
                  onClick={() => setSel(l)}
                  className="border-t border-border hover:bg-noir-elevated cursor-pointer"
                >
                  <td className="p-3">
                    <div className="font-medium">{l.name}</div>
                    <div className="text-xs text-muted-foreground">{l.occupation || "—"}</div>
                  </td>
                  <td className="font-mono text-gold">
                    {formatNaira(l.budget_max || l.budget_min)}
                  </td>
...
function NewLeadSheet({ onSaved }: { onSaved: () => void }) {
  ...
  const save = async () => {
    ...
    await supabase.from("leads").insert({
      user_id: user.id,
      ...f,
      budget_max: Number(f.budget_max) || null,
      intent_score: Number(f.intent_score),
    });
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="bg-noir-elevated border-gold/30 overflow-y-auto">
        <SheetHeader>
          <SheetTitle className="font-serif">New Lead</SheetTitle>
        </SheetHeader>
        <div className="space-y-3 mt-4">
          <FieldEl label="Name">
            <Input value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} />
          </FieldEl>
          <FieldEl label="Occupation">
            <Input
              value={f.occupation}
          ...
          <FieldEl label="Location preference">
            <Input
              value={f.location_preference}
              onChange={(e) => setF({ ...f, location_preference: e.target.value })}
            />
          </FieldEl>
          <FieldEl label="Source">
            <Select value={f.source} onValueChange={(v) => setF({ ...f, source: v })}>
              ...
              <SelectContent>
                {["WhatsApp", "Meta Ads", "Referral", "Direct"].map((s) => (
                  <SelectItem key={s} value={s}>
                    {s}
                  </SelectItem>
                ))}
          ...
          <FieldEl label="Intent score">
            <Input
              type="number"
              min={0}
              max={100}
              value={f.intent_score}
              onChange={(e) => setF({ ...f, intent_score: Number(e.target.value) })}
            />
          </FieldEl>
          <FieldEl label="Stage">
...
function LeadDetailOverlay({ lead, onClose }: { lead: any; onClose: () => void }) {
  ...
  const send = async () => {
    ...
    await supabase.from("conversations").insert({
      ...
      role: "ai",
      message_text: draft,
      annotation: paused ? "↳ Manual override (CEO)" : "↳ AI auto-send",
    });
    setDraft("");
    load();
  };
        ...
        <div className="grid lg:grid-cols-[240px_1fr_270px] min-h-[70vh]">
          ...
          <aside className="p-5 border-r border-border space-y-4">
            <div className="text-center">
              <div className="w-20 h-20 rounded-full bg-gold flex items-center justify-center text-noir font-serif text-3xl mx-auto mb-2">
                {lead.name?.[0]}
              </div>
            ...
            <div className="text-center">
              ...
              <div className="font-mono text-5xl text-gold gold-text-glow">
                {lead.intent_score || 0}
              </div>
              <div className="mt-2">
              ...
              <div className="text-xs text-muted-foreground mt-2 font-mono">
                Last active {timeAgo(lead.last_touch_at)}
              </div>
            </div>
          </aside>
          ...
          <section className="flex flex-col">
                ...
                convo.map((m) => (
                  ...
                  >
                    <div className="max-w-[75%]">
                      <div
                        className={`px-3 py-2 rounded-xl text-sm ${m.role === "ai" ? "bg-gold/15 border border-gold/25" : "bg-noir-elevated"}`}
                      >
                        {m.message_text}
                      </div>
                      {m.role === "ai" && m.annotation && (
                        <div className="text-[10px] font-mono text-gold mt-1 ml-2">
                          {m.annotation}
                        </div>
                      )}
                    </div>
            ...
            <div className="border-t border-border p-3 flex gap-2">
              <Input
                ...
                placeholder={
                  paused ? "Manual override active — type message…" : "Send AI follow-up…"
                }
                onKeyDown={(e) => e.key === "Enter" && send()}
              />
              <Button onClick={send} className="bg-gold text-noir">
          ...
          <aside className="p-5 border-l border-border space-y-3 bg-noir/40">
            ...
            <ConsoleCard
              ...
              body={lead.current_objective || "—"}
            />
            <ConsoleCard
              icon={<Brain className="w-4 h-4 text-gold" />}
              title="Psychological Profile"
              body={lead.psych_profile || "Not yet inferred."}
            />
            <ConsoleCard
              accent="gold"
              title="Next Recommended Move"
              body={lead.next_move || "Awaiting first AI analysis after next inbound message."}
            />
            <ConsoleCard
              accent="warning"
...
function ConsoleCard({ icon, title, body, accent }: any) {
  ...
  return (
    <div
      className={`rounded-xl p-3 border ${colorMap[accent] || "border-border bg-noir-elevated/40"}`}
    >
      ...
      <div
        className={`text-sm ${accent === "gold" ? "text-gold" : accent === "warning" ? "text-warning" : ""}`}
      >
        {body}
      </div>
    </div>
