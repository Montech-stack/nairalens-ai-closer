      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Every plot. Every status. One source of truth.
          </p>
        </div>
        <Sheet open={open} onOpenChange={setOpen}>
          ...
          <SheetContent className="bg-noir-elevated border-gold/30 overflow-y-auto">
            <SheetHeader>
              <SheetTitle className="font-serif">Add Property</SheetTitle>
            </SheetHeader>
            <div className="space-y-3 mt-4">
              <FieldEl label="Plot ID">
                <Input
                  value={form.plot_id}
              ...
              <FieldEl label="Title type">
                <Select
                  ...
                  onValueChange={(v) => setForm({ ...form, title_type: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {["C of O", "Gazette", "Survey Plan", "Other"].map((c) => (
      ...
      <div className="md:hidden space-y-3">
        {list.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground text-sm">
            No properties yet. Click Add Plot.
          </div>
        ) : (
          list.map((p) => (
            <div key={p.id} className="bg-noir-elevated rounded-xl p-4 border border-border">
              ...
              <div className="space-y-1 text-xs text-muted-foreground">
                ...
                <div>
                  ...
                  <span className="text-gold font-mono">{formatNaira(p.price)}</span>
                </div>
                  ...
                  <td>
                    <div className="flex items-center gap-2">
                      <label
                        ...
                        title={p.document_url ? "Replace document" : "Upload document"}
                      >
...
function StatusBadge({ s }: { s: string }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded text-[10px] font-mono border uppercase ${map[s] || ""}`}>
      {s}
    </span>
  );
}
  ...
  return (
    <section className="glass rounded-2xl p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="font-serif text-2xl flex items-center gap-2">
            <ShieldAlert className="w-5 h-5 text-gold" /> Objection Kill-Switch
          </h2>
          <p className="text-sm text-muted-foreground">
            Pre-program responses. AI deploys instantly.
          </p>
        </div>
        <Sheet open={open} onOpenChange={setOpen}>
          <SheetTrigger asChild>
          ...
          <SheetContent className="bg-noir-elevated border-gold/30">
            <SheetHeader>
              <SheetTitle className="font-serif">New Objection Rule</SheetTitle>
            </SheetHeader>
            <div className="space-y-3 mt-4">
              <FieldEl label="If they say">
                <Input
              ...
              <FieldEl label="Respond with">
                <Textarea
                  rows={5}
                  value={form.response_script}
                  onChange={(e) => setForm({ ...form, response_script: e.target.value })}
                  placeholder="Sir, our pricing reflects..."
                />
              </FieldEl>
              <Button onClick={save} className="w-full bg-gold text-noir hover:bg-gold/90">
                Save Rule
      ...
      <div className="grid md:grid-cols-2 gap-3">
        {rules.length === 0 ? (
          <div className="md:col-span-2 text-center py-8 text-muted-foreground text-sm">
            No rules yet.
          </div>
        ) : (
          rules.map((r) => (
            <div key={r.id} className="glass rounded-xl p-4 border-gold/20">
              ...
              <button
                onClick={() => del(r.id)}
                className="mt-3 text-xs text-danger hover:underline flex items-center gap-1"
              >
                <Trash2 className="w-3 h-3" /> Remove
              </button>
            </div>
