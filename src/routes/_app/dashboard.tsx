...
function Dashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const [{ data: leadsData }, { data: sold }, { count: objCount }, { count: aiCount }] =
        await Promise.all([
      ...
      <div className="glass-gold rounded-2xl p-8 gold-glow">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <div className="font-serif text-[52px] leading-none text-gold gold-text-glow">
              {compactNaira(stats.revenue)}
            </div>
            <div className="mt-4 flex flex-wrap gap-x-8 gap-y-2 text-sm">
              <Stat label="Deals Closed" value={String(stats.deals)} />
              <Stat
                label="Avg. Close Time"
                value={stats.avgCloseDays ? `${stats.avgCloseDays} days` : "—"}
              />
          ...
          <Dialog>
            ...
            <DialogContent className="bg-noir-elevated border-gold/30">
              <DialogHeader>
                <DialogTitle className="font-serif">Broadcast Market Pulse</DialogTitle>
              </DialogHeader>
              <p className="text-sm text-muted-foreground">
                A live fact injected into every active AI session right now.
              </p>
              <Textarea
                value={marketPulse}
                onChange={(e) => setMarketPulse(e.target.value)}
                placeholder="e.g. CBN raised rates to 27.5% — buyers acting fast."
                rows={4}
              />
              <Button onClick={broadcastPulse} className="bg-gold text-noir hover:bg-gold/90">
        ...
        <div className="space-y-3">
          {leads.length === 0 ? (
            ...
            leads.slice(0, 8).map((l) => (
              <button
                key={l.id}
                onClick={() => navigate({ to: "/leads", search: { lead: l.id } as any })}
                  ...
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs text-muted-foreground font-mono mt-0.5">
                        ⚡ {l.current_strategy || "Discovery — qualifying intent"}
                      </div>
                    </div>
                  </div>
                  <div className="text-xs text-muted-foreground shrink-0 font-mono">
                    {timeAgo(l.last_touch_at)}
                  </div>
                </div>
              </button>
            ))
...
function EmptyState() {
  return (
    <div className="text-center py-12 text-muted-foreground">
      ...
      <p className="text-sm">
        No active negotiations yet. Connect WhatsApp to start receiving leads.
      </p>
    </div>
  );
}
